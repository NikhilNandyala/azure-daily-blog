...
<DiagramImage src="https://res.cloudinary.com/ybcsc7rq/image/upload/f_auto,q_auto/enterprise-portal-architecture-v2_qvdqsw" alt="Enterprise Customer Portal full architecture: Front Door → App Gateway WAF → VNet (App Service + .NET API + Azure SQL private endpoint) + shared services + Azure DevOps CI/CD pipeline" caption="Full architecture: Front Door Premium → App Gateway WAF_v2 → VNet with App Services (React + .NET 8) → Azure SQL private endpoint. Shared services + CI/CD pipeline below." />
